...
const Tree = ({
    ...
    if (creating) {
        return (
            <>
                <button onClick={() => setIsOpen((prev) => !prev)} className="group flex items-center gap-1 h-5.5 hover:bg-accent/30 w-full" style={{
    ...
    return (
        <>
            <TreeItemWrapper item={item} level={level} onClick={() => { setIsOpen((prev) => !prev) }} onRename={() => setIsRenaming(true)} onDelete={() => {
                deleteFile({ id: item._id })
            }} onCreateFile={() => startCreating("file")} onCreateFolder={() => startCreating("folder")}>
                {folderRender}
            </TreeItemWrapper>
            {
                isOpen && (
