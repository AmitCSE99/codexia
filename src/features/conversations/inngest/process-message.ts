...
export const processMessage = inngest.createFunction(
    {
        ...
        onFailure: async ({ event, step }) => {
            ...
            const internalKey = process.env.CODEXIA_CONVEX_INTERNAL_KEY;
            ...
            if (internalKey) {
                await step.run("update-message-on-failure", async () => {
                    await convex.mutation(api.system.updateMessageContent, {
                        internalKey,
                        messageId,
                        content: "My Apologies, I have encountered an error while trying to process your request"
                    })
                })
            }
        }
    ...
    async ({ event, step }) => {
        ...
        if (!internalKey) {
            throw new NonRetriableError("CODEXIA_CONVEX_INTERNAL_KEY is not configured!")
        }
        ...
        const network = createNetwork({
            ...
            router: ({ network }) => {
                ...
                //Only stop if there is TEXT without tool calls
        ...
        let assistantResponse = "I processed your request. Let me know if you anything else";
