...
const validateInternalKey = (key: string) => {
    ...
    if (!internalKey) {
        throw new Error("CODEXIA_CONVEX_INTERNAL_KEY is not configured")
    }
...
export const renameFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (existing) {
            throw new Error(`A ${file.type} named ${file.name} already exists`);
        }
        ...
        const deleteRecursive = async (fileId: typeof args.fileId) => {
            ...
            if (item.type === "folder") {
                const children = await ctx.db
                    .query("files")
                    .withIndex("by_project_parent", (q) => q.eq("projectId", item.projectId).eq("parentId", item.parentId))
                    .collect();
